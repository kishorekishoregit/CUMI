jQuery(function ($) {
    ...
    $('#DASHBOARD,#MASTER,#TRANSACTION,#USER,#PRINT,#REPORT').hide();

   

    $('#DAS001,#MAS001,#MAS002,#MAS003,#MAS004,#MAS005,#MAS006,#MAS007,#MAS008,#TRA001,#TRA002,#TRA003,#TRA004,#TRA005,#TRA006,#TRA007,#TRA008,#TRA009,#TRA010,#TRA011,#TRA012,#UAC001,#UAC002,#PRT001,#PRT002,#PRT003,#PRT004,#PRT005,#RPT001,#RPT002,#RPT003,#RPT004,#RPT005,#RPT006,#RPT007,#RPT008,#RPT009,#RPT010,#RPT011,#RPT012,#RPT013,#RPT014,#RPT015,#RPT016,#RPT017,#RPT018,#RPT019,#RPT020,#RPT021,#RPT022,#RPT023,#RPT024,#RPT025,#RPT026,#RPT027,#RPT028,#RPT029,#RPT030,#RPT031,#RPT032,#RPT033').hide();
    ...
    function ScreenDetails(Userdetails, Screendetails) {
        ...
        $.each(Screendetails, function (i, v) {
            ...
            if (v["FUNCTIONNAME"] == "USER") { $('#USER').show(); }
            if (v["FUNCTIONNAME"] == "REPORT") { $('#REPORT').show(); }
            if (v["FUNCTIONNAME"] == "PRINT") { $('#PRINT').show(); }
    ...
    $.ajax({
        url: '../Admin/Menu',
        type: "POST",
        data: { 'loginuser': $("#loginuser").text() },
        // data: formData,
        success: function (data) {
            buildMenu($('#menuul'), JSON.parse(data));
            breadcrumb();
        }
    });
