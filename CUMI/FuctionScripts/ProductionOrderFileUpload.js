jQuery(function ($) {
    $('#btnupload').click(function (result) {
        ...
        if (fileUpload.value != null) {
            ...
            if (files.length > 0) {
                ...
                $('.loadingGIF').show();
                $.ajax({
                    url: 'ProductionOrderFileUploadView',
                    contentType: false,
                    processData: false,
                    data: uploadFile,
                    type: 'POST',
    ...
    function BindTab(ResData, type) {
        ...
        if (exampleRecord) {
            ...
            $('#dynamic-tableChild').dataTable({
                "language": {
                    "emptyTable": "Production Order File Upload No Records Found.."
                },
                'bSort': false,
                'aoColumns': [
    ...
    $("#btnsubmit").click(function () {
        ...
        if (files.length > 0) {
            ...
            $('.loadingGIF').show();
            $.ajax({
                url: 'ProductionOrderFileUploadInsert',
                contentType: false,
                processData: false,
                data: uploadFile,
                type: 'POST',
    ...
    $('#btntemplateimport').click(function () {
        ...
        window.location.href = "ProductionOrderFileUploadDownload/?file=ProductionOrder.xlsx";
    });
});