jQuery(function ($) {
    ...
    function BindTable() {
        ...
        $.ajax({
            ...
            success: function (Result) {
                ...
                var CustomerName = JSON.parse(pageload[0]);
                var Reverse = JSON.parse(pageload[1]);



                $("#ddlcustomername").empty().append('<option value="">Select an Option</option>');
                $.each(CustomerName, function () {
                    $("#ddlcustomername").append($("<option></option>").val(this["CUSTOMERNAME"]).html(this["CUSTOMERNAME"]));
                    $('#ddlcustomername').trigger("chosen:updated");
                });
                $('#txtReverseTransporterName').val(Reverse[0].REVERSELOGISTICNAME);
                //BindTab(Details, '1');
            }
        });
        $(".loader").hide("slow");
        ...
        if (exampleRecord) {
            ...
            $('#dynamic-table').dataTable({
                ...
                columns: [
                    ...
                    {
                        className: "text-capitalize", 'data': 'Customer Name',
                    ...
                    { className: "text-capitalize", 'data': 'Reverse Transporter Name' },
                    { className: "text-capitalize", 'data': 'Truck No' },
                    { className: "text-capitalize", 'data': 'From Date' },
                    { className: "text-capitalize", 'data': 'To Date' },
                    {
    ...
    $("#ddlcustomername").change(function () {
        $.ajax({
            url: 'PickUpinformationgenerate',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: $.param({ 'CUSTOMERNAME': $('#ddlcustomername').val().trim() }),
            success: function (data) {
                var Res = data.split('|');
                var Destination = JSON.parse(Res[0]);
                
                $('#txtDestination').val(Destination[0].CITY);
    ...
    $('#PickUp-form').parsley().on('field:validated', function () {
    ...
    function save() {
        ...
        if (window.FormData !== undefined) {
            ...
            formData.append('CUSTOMERNAME', $('#ddlcustomername').val());
            formData.append('DESTINATION', $('#txtDestination').val());
            formData.append('REVERSETRANSPORTER', $('#txtReverseTransporterName').val());
