jQuery(function ($) {
    ...
    function BindTab(ResData, type) {
        ...
        if (exampleRecord) {
            ...
            $('#dynamic-tableChild').dataTable({
                "language": {
                    "emptyTable": "RDC Picklist File Upload No Records Found.."
                },
                'bSort': false,
                'aoColumns': [
    ...
    $("#btnsubmit").click(function () {
        ...
        if (files.length > 0) {
            ...
            $('.loadingGIF').show();
            $.ajax({
                url: 'RDCPicklistFileuploadInsert',
                contentType: false,
                processData: false,
                data: uploadFile,
                type: 'POST',
    ...
    $('#btntemplateimport').click(function () {
        ...
        window.location.href = "RDCPicklistFileUploadDownload/?file=RDCPicklist.xlsx";
    });
});