                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    using (SqlConnection con = new SqlConnection(connectionstring))
                    {
                        ...
                        if (ds != null)
                        {
                            ...
                            response.JS_UserCreationDetails = ds.Tables[1];  // get the User Creation Details
                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    using (SqlConnection con = new SqlConnection(connectionstring))
                    {
                        ...
                        if (ds != null)
                        {
                            ...
                            response.ErrorContainer.Add(new ErrorItem { ErrorNo = ds.Tables[0].Rows[0][1].ToString(), DataItem = request.requestUserCreation.UserName.ToUpper() });
                        }
                    }
                    scope.Complete();
                }
