            ...
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    using (SqlConnection con = new SqlConnection(connectionstring))
                    {
                        ...
                        if (ds != null)
                        {
                            if (ds.Tables[0].Rows[0][0].ToString() == "SUCCESS")
                            {
                                foreach (UserAuthenticationEntity req in request.requestUserRoleScreenDetails)
                                {
                                    ...
                                    cmd1.Parameters.AddWithValue("@USERVIEW", req.view);
                                    cmd1.Parameters.AddWithValue("@Cancel", req.Cancel);
                                    cmd1.Parameters.AddWithValue("@HOD", req.HOD);
                        ...
                        //if (ds.Tables[0].Rows[0][0].ToString() == "SUCCESS" && request.requestCutBoardInwarddetails.Count == rowcount)
                        //{
                        //    scope.Complete();
                        //    response.result = true;
                        //    response.ErrorContainer.Add(new ErrorItem { ErrorNo = ds.Tables[0].Rows[0][1].ToString(), DataItem = ds.Tables[0].Rows[0][2].ToString() });
                        ...
                        //    //  response.JS_UserAuthenticationDetails = DataTableToJSONWithJavaScriptSerializer(ds.Tables[0]);
                        //    response.ErrorContainer.Add(new ErrorItem { ErrorNo = ds.Tables[0].Rows[0][1].ToString(), DataItem = ds.Tables[0].Rows[0][2].ToString().ToUpper() });
                        //}
                    }
                }
            }
